...
# sidecar, both arch-specific). Bundled so the one SRPM builds on every arch
# chroot; extracted whole in %%prep via %%ifarch so BOTH the binary and the
# sidecar are swapped. Same version macro => the watcher bumps both in lockstep.
# On the Debian lane it travels as a Debtransform-Files extra swapped by rules.
...
%changelog
* Sat Jul 18 2026 gmipf <gmipf64@gmail.com> - 5.4.2-8
- Add aarch64 (arm64) support. Bundle the upstream linux_arm64 tarball (NativeAOT
  ...
  matching one whole per build arch via %%ifarch; ExclusiveArch is now x86_64
  aarch64. packit builds ONE SRPM that COPR/OBS build across every arch chroot,
  so bundling both and selecting at %%prep is the only correct way. The build-time
  manpage generator runs the arm64 binary natively on the aarch64 builders (the
  hard guard still fails loudly on any silent fallback). arm64 ships UNTESTED --
  no hardware drive-access proof exists for it; the repackaging path is
  architecture-neutral but this is deliberately not claimed as proven.
